...
2018-09-20 AA. Revised and modified for LCO use. 
'''
...
class ScriptClass(object,):
    def __init__(self, sr, ):
        ...
        ss = "Time    Inst     RA     DEC    Rot        axisErr         GuideRMS" 
        self.logWdg1.addMsg("%s" % (ss), tags=["b","cur"])
        self.logWdg1.addMsg("%s" % (sw),tags=["b","cur"])

        self.logWdg2.addMsg("--- Focus ---", tags=["g","cur"])
        ss="Time    Inst     ThreadRingPos     M2       Alt    secTrussTemp   seeing  Airmass"
        self.logWdg2.addMsg("%s" % (ss,), tags=["g","cur"])
        self.logWdg2.addMsg("%s" % (sw),tags=["g","cur"])
                                
        self.logWdg3.addMsg("--- TCC Temps ---", tags=["cur"])
        ss="Time %s Inst    Outside Inside Primary Cell Floor xyz  Truss   Reserved" % (2*s,)
...
#              time.gmtime(time.time() -  - RO.Astro.Tm.getUTCMinusTAI()))
    ...
    def record(self,sr,atm):
      ...
      secTrussTemp=sr.getKeyVar(self.tccModel.secTrussTemp, ind=0, defVal=99)
      ThreadRingPosition=sr.getKeyVar(self.tccModel.threadringEncPos, ind=0, defVal=99.99)
      secF=sr.getKeyVar(self.tccModel.secFocus, ind=0, defVal=99.99)
      Outside=sr.getKeyVar(self.tccModel.tccTemps, ind=0, defVal=99.99)
      Inside=sr.getKeyVar(self.tccModel.tccTemps, ind=1, defVal=99.99)
      Primary=sr.getKeyVar(self.tccModel.tccTemps, ind=2, defVal=99.99)
      Cell=sr.getKeyVar(self.tccModel.tccTemps, ind=3, defVal=99.99)
      Floor=sr.getKeyVar(self.tccModel.tccTemps, ind=4, defVal=99.99)
      xyz=sr.getKeyVar(self.tccModel.tccTemps, ind=5, defVal=99.99)
      Truss=sr.getKeyVar(self.tccModel.tccTemps, ind=6, defVal=99.99)
      Reserved=sr.getKeyVar(self.tccModel.tccTemps, ind=7, defVal=99.99)
      ...
      ra = RO.CnvUtil.posFromPVT(self.tccModel.objNetPos[0])
      dec = RO.CnvUtil.posFromPVT(self.tccModel.objNetPos[1])
      #airmass
      airmass=sr.getKeyVar(self.tccModel.airmass, ind=0,defVal=99.9 )
      raErr=sr.getKeyVar(self.guiderModel.axisError, ind=0,defVal=99.9 )
      decErr=sr.getKeyVar(self.guiderModel.axisError, ind=1,defVal=99.9 )
      guideRMS=sr.getKeyVar(self.guiderModel.guideRMS, ind=1,defVal=99.9 )

      fwhm=sr.getKeyVar(self.guiderModel.fwhm, ind=4,defVal=99.9 )
      seeing=sr.getKeyVar(self.guiderModel.seeing, ind=0,defVal=99.9 )
   ...
   #   dustb="%5s" % (sr.getKeyVar(self.apoModel.dustb, ind=0, defVal="n/a"))
      ...
      ss="%s %s %4.2f %4.2f %6.2f  %6.4f  %6.4f    %6.2f" % (tm,cart, ra, dec, rot, raErr, decErr, guideRMS)  
      self.logWdg1.addMsg("%s" % (ss), tags=["b","cur"])
      
      # focus
      ss1="%s %s  %8.2f       %8.2f" % (tm,cart,ThreadRingPosition,secF)
      ss2="   %4.1f      %5.1f          %3.1f   %6.2f"  %  (alt, secTrussTemp,seeing, airmass)
      ss=ss1+ss2
      self.logWdg2.addMsg("%s" % (ss), tags=["g","cur"])
      
      # weather
      ss1="%s %s  %5.1f  %5.1f   %5.1f  %s"  % (tm, cart, Outside, Inside, Primary, Cell,)
      ss2="  %s  %s %s  %5.1f  " % (Floor,xyz, Truss, Reserved)
      ss=ss1+ss2      
      self.logWdg3.addMsg("%s " % (ss), tags=["cur"] )
